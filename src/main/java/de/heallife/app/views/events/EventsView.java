...
public class EventsView extends FlexLayout {
  ...
  VerticalLayout buildFormLayout() {
    ...
    var descriptionY =
        new Paragraph(
            "bietet sich als Stressentlastungs-Übung an und wird auch in der Behandlung von"
                + " Patienten mit Burn-out integriert werden.");
    ...
    var descriptionG =
        new Paragraph(
            "Motiviert, kraftvoll und gesund in den Tag starten, zusammen mit Ina und einer Ladung"
                + " Energie.");

    subtitleG.addClassName("subtitle");
    descriptionG.addClassName("description");

    yogaNidra.addClickListener(
        event -> {
          String route = RouteConfiguration.forSessionScope().getUrl(PostView.class, 235109);
          layout.getUI().ifPresent(ui -> ui.navigate(route));
        });

    gutenMorgen.addClickListener(
        event -> {
          String route = RouteConfiguration.forSessionScope().getUrl(PostView.class, 235698);
          layout.getUI().ifPresent(ui -> ui.navigate(route));
        });
  ...
  VerticalLayout buildEventsLayout() {
    ...
    var descriptionY =
        new Paragraph(
            "Hier findest du alle Live Angebote in deiner Umgebung zum Teilnehmen. Hier ist"
    ...
    local.addClickListener(
        event -> {
          String route = RouteConfiguration.forSessionScope().getUrl(CategoryView.class, "Events");
          layout.getUI().ifPresent(ui -> ui.navigate(route));
        });
    ...
    var description =
        new Paragraph("Hier findest du alle Live Angebote die über das Internet stattfinden.");
    ...
    online.addClickListener(
        event -> {
          String route =
              RouteConfiguration.forSessionScope().getUrl(CategoryView.class, "Events_Online");
          layout.getUI().ifPresent(ui -> ui.navigate(route));
        });
